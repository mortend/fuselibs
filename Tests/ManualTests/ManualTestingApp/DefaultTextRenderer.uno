//a slightly modified form of the old DefaultTextRenderer
using Uno;
using Uno.Graphics;
using Uno.UX;
	...
	public class DefaultTextRenderer
	{
		...
		static TextRenderer renderer
		{
			get
			{
				if (_renderer == null) 
				{
					_renderer = new TextRenderer(initialMaxCharCount, new SpriteFontShader());
		...
		static void EnsureRendererCapacity(int maxCharCount)
		{
			...
			_renderer = new TextRenderer(_maxCharCount, new SpriteFontShader());
			_renderer.Transform = new ProperTextTransform();
		}
	}
